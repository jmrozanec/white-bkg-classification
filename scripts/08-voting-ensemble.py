...
eclf = VotingClassifier(estimators=clfs, voting='soft', weights=[0.230,0.125,0.322,0.322])
scores = cross_val_score(eclf, dataset.data, dataset.target, cv=5)
print("%s, accuracy: %0.4f (+/- %0.4f)" % ("ensemble", scores.mean(), scores.std() * 2))
eclf = VotingClassifier(estimators=clfs, voting='hard', weights=[0.230,0.125,0.322,0.322])
scores = cross_val_score(eclf, dataset.data, dataset.target, cv=5)
print("%s, accuracy: %0.4f (+/- %0.4f)" % ("ensemble", scores.mean(), scores.std() * 2))
